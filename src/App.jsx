...
function EditableRuleList({ rules, onUpdate, filterEnvId }) {
  ...
  const [showEnvPicker, setShowEnvPicker] = useState(null); // "edit" | "new" | null

  const filtered = rules.map((r, i) => ({ r, i })).filter(({ r }) => ruleMatch(r, filterEnvId));
  const add = () => { if (!draft.trim()) return; onUpdate([...rules, { text: draft.trim(), envIds: [...draftEnvIds] }]); setDraft(""); setDraftEnvIds([]); };
  const remove = (i) => { onUpdate(rules.filter((_, idx) => idx !== i)); setConfirmDel(null); };
  const saveEdit = () => { if (editIdx === null) return; const n = [...rules]; n[editIdx] = { ...n[editIdx], text: editText, envIds: [...editEnvIds] }; onUpdate(n); setEditIdx(null); setShowEnvPicker(null); };
          ...
          {confirmDel === i ? (
            ...
            <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2, flexShrink: 0 }}>
                <button style={{ background: "none", border: "none", color: C.accent, fontSize: 11, cursor: "pointer", padding: "2px 4px" }} onClick={() => { setEditIdx(i); setEditText(r.text); setEditEnvIds([...(r.envIds || [])]); setConfirmDel(null); setShowEnvPicker(null); }}>✎</button>
