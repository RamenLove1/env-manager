...
const ENVIRONMENTS = [
  ...
  { id: "jikka2", label: "実家系(モード2)", icon: "🏡", cost: "0円", hours: "帰省時", access: "帰省時",
    ...
    gray: [],
    notes: "家(モード2)と同じ扱い。家(モード2)の貯蔵を使用する。",
...
const DEFAULT_PERIOD_RULES = [
  {
    ...
    effects: [
      ...
      { envId: "jikka2", type: "storage_threshold", storageKey: "home2", threshold: 180, label: "家(モード2)貯蔵3h以上で開始可能" },
...
function loadDyn() {
  try { const r = localStorage.getItem("env-mgr-dyn-v6"); return r ? JSON.parse(r) : null; } catch { return null; }
}
function saveDyn(d) {
  try { localStorage.setItem("env-mgr-dyn-v6", JSON.stringify(d)); } catch {}
}
...
function collectEffects(dyn) {
  ...
  (dyn.periodRules || []).filter(isPeriodActive).forEach(p => (p.effects || []).forEach(e => add(e, `📅${periodDisplayLabel(p)}`)));
  (dyn.controlRules || []).filter(isPeriodActive).forEach(p => (p.effects || []).forEach(e => add(e, `🔧${periodDisplayLabel(p)}`)));
  return map;
}
  ...
  return (
    <div>
          ...
          <div>
            ...
            {cur.notes && (
              <div style={{ marginTop: 8, padding: "8px 10px", borderRadius: 8, background: C.bg, border: `1px solid ${C.yellow}44`, lineHeight: 1.6, whiteSpace: "pre-line" }}>
                <div style={{ fontSize: 10, color: C.yellow, marginBottom: 4, fontWeight: 600 }}>📌 備考</div>
                <div style={{ fontSize: 12, color: C.text }}>{cur.notes}</div>
              </div>
            )}
          </div>
      ...
      <div style={{ marginTop: 16 }}>
        <Toggle title="🚶 起床・外出後の選択肢" defaultOpen={true}>
...
const EFFECT_TYPES = [
  ...
  { id: "storage_threshold", label: "⚠️ 貯蔵で許可（固定値）" },
  { id: "time_ratio", label: "🕐 貯蔵で許可（時間比率）" },
];
...
function EffectsEditor({ effects = [], onUpdate }) {
  ...
  const add = () => {
    ...
    if (newType === "denied") {
      ...
      if (!newKey || !newNum) return;
      eff.storageKey = newKey; eff.k = parseFloat(newNum) || 2; eff.label = newLabel || `24時まで k=${newNum} で許可`;
    }
  ...
  const describeEffect = (eff) => {
    ...
    if (eff.type === "time_ratio") {
      const sk = STORAGE_KEYS.find(s => s.key === eff.storageKey);
      return `時間比率: ${eff.label}（${sk?.label || eff.storageKey}, k=${eff.k}）`;
    }
    return `不明な影響タイプ`;
  };
  ...
  return (
    <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 10, paddingTop: 8 }}>
      ...
      <div style={{ marginTop: 8, padding: 10, background: C.bg, borderRadius: 8 }}>
        ...
        {(newType === "count_limit" || newType === "storage_threshold" || newType === "time_ratio") && (
        ...
        {newType === "time_ratio" && (
          <>
            ...
            <div style={{ fontSize: 10, color: C.textDim, marginBottom: 6, lineHeight: 1.4 }}>
              n分の貯蔵があるとき、24時まで k×n 分なら許可。必要貯蔵 = 残り時間 ÷ k
            </div>
          </>
        )}
        {newType !== "denied" && (
...
function RulesTab({ dyn, setDyn }) {
  ...
  const addPeriod = () => { const n = { ...dyn, periodRules: [...dyn.periodRules, { id: Date.now(), label: "新しい期間", startDate: "", endDate: "", guidelines: {}, choices: [], rules: [], effects: [] }] }; setDyn(n); saveDyn(n); };
