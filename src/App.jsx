...
const ENVIRONMENTS = [
  ...
  { id: "home3", label: "家(モード3)", icon: "🏠", cost: "0円", hours: "いつでも", access: "家",
    ...
    gray: ["勉強", "仕事"],
    notes: "風邪をひいているときにのみ、かつその時は常に許可する。\n寝転がる必要がある場合には、スマホでのYoutube視聴を許可する。",
    担当: "療養" },
  { id: "outside", label: "外", icon: "🚶", cost: "0円", hours: "移動時、外食時等", access: "0秒",
    permitted: ["勉強", "仕事", "趣味", "ゲーム"],
    gray: ["勉強", "仕事", "趣味", "ゲーム"],
    notes: "仕事は家庭教師関連のもののみ。\nゲームは少量のもののみ。",
    担当: "環境移動" },
  ...
  { id: "office", label: "オフィス", icon: "🏢", cost: "0円", hours: "10時~22時", access: "徒歩20分",
    ...
    notes: "推定努力密度：100%",
    担当: "仕事" },
  { id: "starbucks", label: "作業可能飲食店", icon: "☕", cost: "2時間500~1000円(注文による)", hours: "スタバ推奨7時~21時/マック推奨21時~7時", access: "徒歩20分",
    permitted: ["勉強", "仕事", "ゲーム"],
    gray: ["仕事", "ゲーム"],
    notes: "仕事は家庭教師関連のもののみ。\nゲームは少量のもののみ。\n推定努力密度：75%\n月間利用料目安：8000円(12回)",
    担当: "勉強(短時間)" },
  ...
  { id: "kaikatsu1", label: "快活クラブ(モード1)", icon: "📚",
    ...
    permitted: ["勉強", "ゲーム", "漫画", "Youtube"],
    gray: ["ゲーム"],
    notes: "1日の最初の1回は勉強1時間で報酬、その後は勉強2時間で報酬。\n報酬は、漫画1巻または50分以内のYoutube動画1本。\nYoutubeを見た回で違反が起きた場合、Youtubeによる違反とする。\nゲームは少量のもののみ。\n推定努力密度：50%\n月間利用料目安：2000円",
    担当: "報酬付き勉強" },
...
const BASIC_RULES_SUBS = [
  { key: "system", title: "システム運用", rules:[
    ...
    R("日付の変更は、0時以降に起きた時、もしくは寝なければいけない時間に行う。"),
    R("期間別規定が終わるまでに、次の期間別規定を作らなければならない。"),
    R("家で許可されているモードがない時でも、無モード状態として睡眠と外出準備のみ許可する。"),
    R("帰省時など、普段の環境から離れる場合、システムは一時停止する。"),
  ]},
  { key: "general", title: "一般", rules: [
    R("スマホでYoutubeを触る場合、音楽を聴くのみにする。それ以外では触ってはいけない。"),
    R("寝室の外にアラームと靴下を設置した場合、もしくはもういつ寝てもよく、いつ起きてもいい状態でのみ、布団に入って良い。（家(モード3)時はいつでも良い）", ["home1","home2","home3"]),
    R("アラームを消すときは立って行う。(家(モード3)時はこの限りではない)", ["home1","home2","home3"]),
    R("寝るまでにやることを考え、時間付きでメモしてからでないと家に帰ってはいけない。"),
    R("娯楽の確認は常に許可する。YouTubeでは、動画が再生されている状態は確認とはみなさない。"),
  ]},
...
const VIOLATION_RULES = [
  R("ある環境、モードで違反が起きた場合、即座に別の環境に移動する(家の場合は寝ても良い)。それもしなかった場合は、Youtubeによる違反と同等に扱う。"),
  R("違反の当日~2日後はその環境、モードは使用不可(家(モード3)、外をのぞく)。家(モード1)の使用を余儀なくされているときは、この限りではない。"),
  R("Youtubeによる違反の場合は、当日~2日後は家、快活クラブへの持ち込みは禁止で、その環境へ移動する前に必ずコインロッカーに預ける。"),
  R("スマホでYoutubeを触ってしまった場合は、当日~2日後は家、快活クラブへの持ち込みは禁止で、その環境へ移動する前に必ずコインロッカーに預ける。"),
  R("家の無モード状態ですぐに別の環境に移動しなかった場合、当日と翌日は家に入ってはいけない。"),
];
...
const PHILOSOPHY = [
  ...
  R("回復期は最初に定めた期間より伸ばさないようにする。"),
  R("外出が必要な日は、朝起きてから外出までその日の予定を考えることはしないようにし、朝食、運動、外出準備のみに徹する。その後どこへ行くか、何をするかは、外出後の自分に任せる。", ["home1","home2","home3"]),
];
