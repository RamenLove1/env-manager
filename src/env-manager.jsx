...
const ENVIRONMENTS = [
  ...
  { id: "kaikatsu1", label: "快活クラブ(モード1)", icon: "📚",
    ...
    担当: "漫画勉強" },
  { id: "kaikatsu2", label: "快活クラブ(モード2)", icon: "📚", cost: "(同上)", hours: "いつでも", access: "徒歩20分",
    permitted: ["趣味", "ゲーム", "漫画", "Youtube"],
    gray: [],
    notes: "12時間以上できるときのみ開始可能。\n月間利用料目安：10000円",
    担当: "完全開放" },
  { id: "kaikatsu3", label: "快活クラブ(モード3)", icon: "📚", cost: "(同上)", hours: "いつでも", access: "徒歩20分",
    permitted: ["ゲーム", "漫画"],
    gray: ["ゲーム", "漫画"],
...
const PLACES = [
  ...
  { key: "starbucks", label: "スタバ", icon: "☕", envIds: ["starbucks"], modeLabels: [] },
...
const BASIC_RULES = [
  ...
  R("重要な時事に関するニュースは、NewsPicks、ヤフーニュースのみなら許可する。"),
  R("システムに関する作業は計測外とする。"),
  R("制御規定では、好きな時に追加規定を入れられる。ただし、報酬などは設定できず、追加規定であっても違反時には違反時規定に従った罰則を受ける。"),
  R("ミッション規定の追加規定では、「テストで100点とる」など、成功を条件にしてはいけない。「勉強を10時間やる」など、やることそのものを条件にする。"),
...
const VIOLATION_RULES = [
  ...
  R("Youtubeによる違反の場合は、即座にPCをコインロッカーに預ける。当日~2日後は家、快活クラブへの持ち込みは禁止で、その環境へ移動する前に必ずコインロッカーに預ける。", ["home1","home2","home3","kaikatsu1","kaikatsu2","kaikatsu3"]),
  R("スマホでYoutubeを見てしまった場合は、即座にスマホをコインロッカーに預ける。当日~2日後は家、快活クラブへの持ち込みは禁止で、その環境へ移動する前に必ずコインロッカーに預ける。", ["home1","home2","home3","kaikatsu1","kaikatsu2","kaikatsu3"]),
  R("快活クラブ(モード1)中にYoutubeをやってしまった場合は、快活クラブからモード2をなくし、機能を宝島に移行させる。", ["kaikatsu1"]),
  R("家にいるが、どのモードでもない（起床直後など）時はすぐに別の環境に移動するべきであるが、これをしなかった場合、当日と翌日は家に入ってはいけない。", ["home1","home2","home3"]),
];
...
const DEFAULT_PERIOD_RULES = [
  {
    id: 1, label: "4/6~4/12(初週)",
    guidelines: { "勉強": "5h", "仕事": "3h", "運動": "30m", "家事": "30m", "趣味": "2h", "リラックス": "1h" },
    rules: [
      R("1h勉強または仕事をするごとに、家(モード2)の許可を10m貯蔵できる。", ["home2"]),
      R("2h勉強または仕事をするごとに、0時までに就寝するごとに、快活クラブ(モード2)の許可を10m貯蔵できる。", ["kaikatsu2"]),
    ...
    // effects: 環境への影響テンプレート
    // type: "denied"(不許可), "count_limit"(回数制限), "storage_threshold"(貯蔵で許可), "conditional"(条件付き/自由記述)
    effects: [
      { envId: "home2", type: "storage_threshold", storageKey: "home2", threshold: 180, label: "貯蔵3h以上で開始可能" },
      { envId: "home2", type: "conditional", label: "大学授業後のみ" },
      { envId: "kaikatsu2", type: "storage_threshold", storageKey: "kaikatsu2", threshold: 1, label: "貯蔵ポイントが必要" },
      { envId: "univ2", type: "denied" },
      { envId: "share2", type: "count_limit", storageKey: "share2_count", limit: 1, label: "月に一度まで" },
    ],
  },
];

const DEFAULT_CONTROL_RULES = [
  // { id: 1, label: "4/6~4/12", rules: [...], effects: [{ envId, status, label }] }
];
...
const PHILOSOPHY = [
  ...
  R("家、大学、SHARE LOUNGE、快活クラブのモード2は、モード1よりも確実に良いものなので、休息日等でなければ許可を条件付きにする。", ["home2","univ2","share2","kaikatsu2"]),
  R("家(モード2)からは簡単には抜けられないので、勉強や仕事が必要な日は、もう寝てもいいような状態になってから許可する。そうすれば、別の環境に移動できなくても、寝てしまえば解決する。", ["home2"]),
  R("家以外の環境でのモチベーション維持のため、努力して帰ってきた場合にのみモード2を許可するようにする。"),
  R("家(モード3)が使用されたあとや、帰省後は、娯楽に対する欲求が一時的に強まるので、制限をかなり緩めた回復期を設ける。", ["home3","jikka1","jikka2"]),
  R("大学やその他の予定がある日の前日は、準備+移動+外出誘発行動で計4hは使うことを想定し、前日の睡眠時刻を合わせる。", ["univ1","univ2"]),
  R("大学やその他の予定が朝早くからある場合に短時間で外出を誘発するために快活クラブ(モード3)を用意してあるので、その用途のみに使う。その際、準備+移動+快活クラブ(モード3)で計2hは使うことを想定し、前日の睡眠時刻を合わせる。", ["kaikatsu3"]),
  R("外出が必要な日は、朝起きてから外出までその日の予定を考えることはしないようにし、朝食、運動、外出準備のみに徹する。その後どこへ行くか、何をするかは、外出後の自分に任せる。"),
];

const RULE_SECTIONS = [
  { key: "basic", title: "📜 基本規程", rules: BASIC_RULES },
...
function loadDyn() {
  try { const r = localStorage.getItem("env-mgr-dyn"); return r ? JSON.parse(r) : null; } catch { return null; }
}
function saveDyn(d) {
  try { localStorage.setItem("env-mgr-dyn", JSON.stringify(d)); } catch {}
}
...
function resolveEffect(eff, storage, source) {
  ...
  return { ...base, blocked: false, color: "yellow", icon: "⚠️", text: eff.label || "条件付き" };
}

// 全effects集約 → { envId: [resolved] }
function collectEffects(dyn) {
  const map = {};
  const st = dyn.storage || {};
  const add = (eff, source) => {
    if (!eff.envId) return;
    if (!map[eff.envId]) map[eff.envId] = [];
    map[eff.envId].push(resolveEffect(eff, st, source));
  };
  (dyn.periodRules || []).forEach(p => (p.effects || []).forEach(e => add(e, `📅${p.label}`)));
  (dyn.controlRules || []).forEach(p => (p.effects || []).forEach(e => add(e, `🔧${p.label}`)));
  return map;
}
...
function RuleList({ rules, filterEnvId }) {
  ...
  return filtered.map((r, i) => (
    <div key={i} style={{ fontSize: 12, lineHeight: 1.6, color: C.text, padding: "6px 0", borderBottom: i < filtered.length - 1 ? `1px solid ${C.border}` : "none" }}>
      {r.text}
      ...
      <div style={S.card}>
        ...
        {cur ? (
          <div>
            ...
            <div style={{ marginTop: 8 }}>{cur.permitted.map(a => <span key={a} style={S.permitted}>{a}</span>)}</div>
            ...
            {cur.notes && <div style={{ color: C.textDim, fontSize: 12, marginTop: 4, lineHeight: 1.5, whiteSpace: "pre-line" }}>{cur.notes}</div>}
          </div>
        ...
        <div style={S.card}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {PLACES.map(p => {
              ...
              const allDenied = p.envIds.every(id => isDenied(id) || isBanned(id, violations));
              const isCur = curPlace?.key === p.key;
              return (
                <button key={p.key} onClick={() => { if (allBanned) return; setSelectedPlace(p); }}
                  style={{ background: isCur ? C.accentDim : C.bg, border: `1px solid ${isCur ? C.accent : allDenied ? C.red+"44" : C.border}`, borderRadius: 10, padding: "14px 10px", cursor: allBanned ? "default" : "pointer", opacity: allBanned ? 0.4 : allDenied ? 0.5 : 1, textAlign: "center", color: C.text }}>
            ...
            {selectedPlace.envIds.map((eid, i) => {
              ...
              return (
                <button key={eid} onClick={() => !blocked && setEnv(eid)}
                  ...
                  <div style={{ marginTop: 6 }}>{env.permitted.map(a => <span key={a} style={S.permitted}>{a}</span>)}</div>
                  {env.担当 && <div style={{ color: C.yellow, fontSize: 11, marginTop: 4 }}>担当: {env.担当}</div>}
                  {envEffects.map((eff, ei) => <EffectBadge key={ei} effect={eff} />)}
                  {env.notes && <div style={{ color: C.textDim, fontSize: 11, marginTop: 4, whiteSpace: "pre-line" }}>{env.notes}</div>}
...
function StorageTab({ dyn, setDyn }) {
  ...
  const TimeCard = ({ id, label, color, steps }) => {
    const mins = st[id] || 0;
    return (
      <div style={S.card}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          {steps.map(s => <button key={s} style={{ ...S.btnOutline, flex: 1 }} onClick={() => adjust(id, s)}>{s > 0 ? "+" : ""}{s >= 60 ? `${s/60}h` : `${s}m`}</button>)}
        </div>
        {editTarget === id ? (
          <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
            <input style={{ ...S.input, flex: 1 }} type="number" placeholder="分" value={amount} onChange={e => setAmount(e.target.value)} />
            <button style={S.btnOutline} onClick={() => setDirect(id)}>設定</button>
            <button style={{ ...S.btnOutline, color: C.textDim, borderColor: C.textDim }} onClick={() => setEditTarget(null)}>×</button>
          </div>
        ) : <button style={{ ...S.btnOutline, marginTop: 8, width: "100%", textAlign: "center" }} onClick={() => { setEditTarget(id); setAmount(String(mins)); }}>直接入力</button>}
      </div>
    );
  };
  ...
  return (
    <div>
      ...
      <TimeCard id="home2" label="家(モード2) 貯蔵" color={C.green} steps={[-10, 10, 60]} />
      <TimeCard id="kaikatsu2" label="快活クラブ(モード2) 貯蔵" color={C.green} steps={[-10, 10, 60]} />
      <div style={S.section}>活動記録</div>
      <TimeCard id="study_mins" label="勉強 合計時間" color={C.accent} steps={[-30, 30, 60]} />
      <TimeCard id="work_mins" label="仕事 合計時間" color={C.accent} steps={[-30, 30, 60]} />
...
function EditableRuleList({ rules, onUpdate, filterEnvId }) {
  ...
  const add = () => { if (!draft.trim()) return; onUpdate([...rules, { text: draft.trim(), envIds: [] }]); setDraft(""); };
  const remove = (i) => { onUpdate(rules.filter((_, idx) => idx !== i)); setConfirmDel(null); };
  const saveEdit = () => { if (editIdx === null) return; const n = [...rules]; n[editIdx] = { ...n[editIdx], text: editText }; onUpdate(n); setEditIdx(null); };
  ...
  return (
    <div>
        ...
        <div key={i} style={{ padding: "6px 0", borderBottom: `1px solid ${C.border}` }}>
          {confirmDel === i ? (
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6, marginTop: 4 }}><button style={S.btnSm()} onClick={saveEdit}>保存</button><button style={S.btnSm(C.textDim)} onClick={() => setEditIdx(null)}>取消</button></div>
            </div>
          ) : (
            <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 2, flexShrink: 0 }}>
                <button style={{ background: "none", border: "none", color: C.accent, fontSize: 11, cursor: "pointer", padding: "2px 4px" }} onClick={() => { setEditIdx(i); setEditText(r.text); setConfirmDel(null); }}>✎</button>
      ...
      <div style={{ marginTop: 10 }}>
        <textarea style={{ ...S.input, fontSize: 12, minHeight: 50, marginBottom: 6 }} value={draft} onChange={e => setDraft(e.target.value)} placeholder="新しい条文を入力..." />
        <div style={{ display: "flex", gap: 8 }}>
          <button style={S.btnSm()} onClick={add}>追加</button>
          <button style={S.btnSm(C.green)} onClick={() => { setEditing(false); setConfirmDel(null); }}>完了</button>
        </div>
      </div>
    </div>
  );
...
// =============================================
const EFFECT_TYPES = [
  { id: "denied", label: "🚫 不許可", fields: [] },
  { id: "count_limit", label: "📏 回数制限", fields: ["storageKey", "limit", "label"] },
  { id: "storage_threshold", label: "⚠️ 貯蔵で許可", fields: ["storageKey", "threshold", "label"] },
  { id: "conditional", label: "📝 条件付き（自由記述）", fields: ["label"] },
];
...
function EffectsEditor({ effects = [], onUpdate }) {
  ...
  const typeDef = EFFECT_TYPES.find(t => t.id === newType);
  ...
  const add = () => {
    ...
    if (newType === "denied") {
      ...
      eff.storageKey = newKey; eff.limit = parseInt(newNum) || 1; eff.label = newLabel || `${parseInt(newNum)}回まで`;
    } else if (newType === "storage_threshold") {
      if (!newKey || !newNum) return;
      eff.storageKey = newKey; eff.threshold = parseInt(newNum) || 0; eff.label = newLabel || `${fmtTime(parseInt(newNum))}以上で開始可能`;
    } else {
      if (!newLabel.trim()) return;
      eff.label = newLabel.trim();
    }
  ...
  const describeEffect = (eff) => {
    ...
    if (eff.type === "storage_threshold") {
      ...
      return `貯蔵で許可: ${eff.label}（${sk?.label || eff.storageKey} ≥ ${thr}）`;
    }
    return `条件付き: ${eff.label}`;
  };
  ...
  return (
    <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 10, paddingTop: 8 }}>
      ...
      <div style={{ marginTop: 8, padding: 10, background: C.bg, borderRadius: 8 }}>
        ...
        {(newType === "count_limit" || newType === "storage_threshold") && (
        ...
        {newType === "storage_threshold" && (
          <input style={{ ...S.input, marginBottom: 6 }} type="number" value={newNum} onChange={e => setNewNum(e.target.value)} placeholder="必要な貯蔵（分）（例: 180 = 3h）" />
        )}
        {newType !== "denied" && (
...
function PeriodBlock({ period, onUpdate, onDelete, filterEnvId, hasGuidelines }) {
  ...
  const saveMeta = () => {
    if (hasGuidelines) {
      try { const g = JSON.parse(guidelines); onUpdate({ ...period, label, guidelines: g }); } catch { alert("JSON形式が不正です"); return; }
    } else {
      onUpdate({ ...period, label });
    }
    setEditMeta(false);
  };

  return (
    <div style={{ background: C.bg, borderRadius: 10, border: `1px solid ${C.border}`, marginTop: 10, overflow: "hidden" }}>
      <button onClick={() => setOpen(!open)} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", background: "none", border: "none", color: C.text, cursor: "pointer" }}>
        <span style={{ fontWeight: 600, fontSize: 13 }}>{period.label}</span>
          ...
          <div style={{ borderTop: `1px solid ${C.border}`, marginTop: 10, paddingTop: 8 }}>
            {!editMeta ? (
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <button style={S.btnSm()} onClick={() => { setLabel(period.label); if (hasGuidelines) setGuidelines(JSON.stringify(period.guidelines || {}, null, 2)); setEditMeta(true); }}>ラベル{hasGuidelines ? "・目安" : ""}を編集</button>
                {!confirmDel ? (
              ...
              <>
                <div style={{ marginBottom: 8 }}>
                  <div style={{ fontSize: 11, color: C.textDim, marginBottom: 4 }}>期間ラベル</div>
...
function RulesTab({ dyn, setDyn }) {
  ...
  const addPeriod = () => { const n = { ...dyn, periodRules: [...dyn.periodRules, { id: Date.now(), label: "新しい期間", guidelines: {}, rules: [] }] }; setDyn(n); saveDyn(n); };
  ...
  const addControl = () => { const n = { ...dyn, controlRules: [...dyn.controlRules, { id: Date.now(), label: "新しい期間", rules: [] }] }; setDyn(n); saveDyn(n); };
  ...
  return (
    <div>
      ...
      <Toggle title="📅 期間別規程" defaultOpen={true}>
        ...
        <button style={{ ...S.btn(), marginTop: 10 }} onClick={addPeriod}>＋ 期間を追加</button>
      </Toggle>
